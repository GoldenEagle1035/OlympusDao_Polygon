...
import { DAI_HOMDAO } from "./AllBonds";
...
export async function getMarketPrice({ networkID, provider }: IBaseAsyncThunk) {
  const DAI_HOMDAO_address = DAI_HOMDAO.getAddressForReserve(networkID);
  const pairContract = new ethers.Contract(DAI_HOMDAO_address, PairContract, provider);
...
export function secondsUntilBlock(startTime: number, endTime: number) {
  const blocksAway = endTime - startTime;
...
function getSbhdTokenImage() {
  return <SvgIcon component={SBhdImg} viewBox="0 0 100 100" style={{ height: "1rem", width: "1rem" }} />;
}

export function getBhdTokenImage(w?: number, h?: number) {
  const height = h == null ? "32px" : `${h}px`;
  const width = w == null ? "32px" : `${w}px`;
  return <SvgIcon component={BhdImg} viewBox="0 0 32 32" style={{ height, width }} />;
}

export function getTokenImage(name: string) {
  if (name === "HOM") return getBhdTokenImage();
  if (name === "sHOM") return getSbhdTokenImage();
}
